...
const Calculator: React.FC = () => {
    ...
    const inputNum = (e: any) => {
        ...
        if (total) {
            // setPreState("");
            dispatch(setPreState(""));
        }

        curState
            ? // ? setCurState((pre) => pre + e.target.innerText)
              // : setCurState(e.target.innerText);
              dispatch(setCurState((pre: any) => pre + e.target.innerText))
            : dispatch(setCurState(e.target.innerText));
        setTotal(false);
    };

    useEffect(() => {
        // setInput(curState);
        dispatch(setInput(curState));
    }, [curState]);
    ...
    const operatorType = (e: any) => {
        ...
        // }
        // setTotal(false);
        dispatch(setTotal(false));
        dispatch(setOperator(e.target.innerText));

        if (curState === "") return;
        if (preState !== "") {
            equals();
        } else {
            dispatch(setPreState(curState));
            dispatch(setCurState(""));
        }
    };
    ...
    const minusPlus = () => {
        if (curState.charAt(0) === "-") {
            dispatch(setCurState(curState.substring(1)));
        } else {
            dispatch(setCurState("-" + curState));
        }
    };
    ...
    const percent = () => {
        preState
            ? dispatch(
                  setCurState(
                      String(
                          (parseFloat(curState) / 100) * parseFloat(preState)
                      )
                  )
              )
            : dispatch(setCurState(String(parseFloat(curState) / 100)));
    };
    // const reset = () => {
    //     setPreState("");
    //     setCurState("");
